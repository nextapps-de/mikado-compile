...
function compile(src_name, dest_name, type) {
  ...
  const template = readFileSync(src_name, "utf8").replace(
    /<!--[\s\S]*?-->/g,
    ""
  );
      ...
      if (!nodes.child.length) {
        ...
        for (let i = 0; i < nodes.child.length; i++) {
          ...
          if (nodes.child[i].node === "text") {
            ...
            if (text.trim()) {
              if (text.indexOf("{{@") !== -1) {
                ...
                text =
                  ...
                  text.substring(text.indexOf("}}", text.indexOf("{{@")) + 2);
              }
            ...
            if (
              nodes.child.length === 1 &&
              (!nodes.text || !nodes.child[i].text) &&
              (!nodes.js || !nodes.child[i].js)
            ) {
            ...
            if (nodes.child[i].attr) {
              ...
              if (nodes.child[i].attr.if) {
                ...
                if (typeof nodes.child[i].if !== "string")
                  nodes.child[i].if = nodes.child[i].if.join("");
              ...
              if (nodes.child[i].attr.key) {
                nodes.child[i]["key"] = nodes.child[i].attr.key.replace(
                  "data.",
                  ""
                );
              ...
              if (nodes.child[i].attr.bind) {
                if (typeof nodes.child[i].attr.bind !== "string")
                  nodes.child[i].attr.bind = nodes.child[i].attr.bind.join("");
              ...
              if (keys.length === 0) {
                ...
                for (let x = 0; x < keys.length; x++) {
                  if (typeof nodes.child[i].attr[keys[x]] === "object") {
                    nodes.child[i].attr[keys[x]] = nodes.child[i].attr[
                      keys[x]
                    ].join(" ");
                  }
                  ...
                  if (
                    !event_types[keys[x]] &&
                    event_types[keys[x].substring(2)] &&
                    nodes.child[i].attr[keys[x]].indexOf("{{") !== -1
                  ) {
                  ...
                  if (event_types[keys[x]]) {
                    ...
                    nodes.child[i]["event"][keys[x]] =
                      nodes.child[i].attr[keys[x]];
  ...
  if (json)
    json =
      type === "json" ? JSON.stringify(json) : JSON.stringify(json, null, 2);
  ...
  if (!type || type === "json")
    writeFileSync(dest_name + ".json", json, "utf8");
  ...
  if (!type || type === "es5") writeFileSync(dest_name + ".js", es5, "utf8");
  if (type === "es5") return es5;

  const es6 = "export default " + json + ";";

  if (!type || type === "es6")
    writeFileSync(dest_name + ".es6.js", es6, "utf8");
